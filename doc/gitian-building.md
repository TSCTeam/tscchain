...
*Setup instructions for a Gitian build of Tsc Core using a Debian VM or physical system.*
...
- [Building Tsc Core](#building-tsc-core)
...
Clone the git repositories for tsc and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/tscproject/tsc --recursive
git clone https://github.com/tscproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/tsc/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the tsc repository under 'Fetch and create inputs' to install sources which require
...
To build Tsc Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the tsc repository.
...
For example:
```bash
URL=https://github.com/laanwj/tsc.git
...
# if wanting to use a different version of eth-cpp-tsc:
./bin/gbuild --commit tsc=${COMMIT},cpp-eth-tsc=${ETHCOMMIT} --url tsc=${URL},cpp-eth-tsc=${ETHURL} ../tsc/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the tsc git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../tsc/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/tscproject/tsc-detached-sigs.git

BTCPATH=/some/root/path/tsc
SIGPATH=/some/root/path/tsc-detached-sigs

./bin/gbuild --url tsc=${BTCPATH},signature=${SIGPATH} ../tsc/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/tsc-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/tsc-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/tsc-osx-build.assert
```
...
[tscproject/gitian.sigs](https://github.com/tscproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@tsc.org) and he will commit them.
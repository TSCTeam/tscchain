...
TscState::TscState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = TscState::openDB(_path + "/tscDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

TscState::TscState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute TscState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, TscTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createTscAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            tsc::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> TscState::vins() const // temp
{
...
void TscState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* TscState::vin(dev::Address const& _a) const
{
    return const_cast<TscState*>(this)->vin(_a);
}

Vin* TscState::vin(dev::Address const& _addr)
{
...
// void TscState::commit(CommitBehaviour _commitBehaviour)
// {
...
void TscState::kill(dev::Address _addr)
{
...
void TscState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address TscState::createTscAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void TscState::deleteAccounts(std::set<dev::Address>& addrs){
...
void TscState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void TscState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;

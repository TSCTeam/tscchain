...
bool TscDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule TscDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t TscDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t TscDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t TscDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t TscDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool TscDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void TscDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void TscDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void TscDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void TscDGP::createParamsInstance(){
...
dev::Address TscDGP::getAddressForBlock(unsigned int blockHeight){
...
void TscDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void TscDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void TscDGP::parseStorageOneUint64(uint64_t& value){
...
void TscDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule TscDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;

...
void initState(){
    ...
    const std::string dirTsc = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<TscState>(new TscState(dev::u256(0), TscState::openDB(dirTsc, hashDB, dev::WithExisting::Trust), dirTsc + "/tscDB", dev::eth::BaseState::Empty));
...
TscTransaction createTscTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout){
    TscTransaction txEth;
    if(recipient == dev::Address()){
        txEth = TscTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = TscTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }

...
void checkResult(bool isCreation, std::vector<TscTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    TscTxConverter converter(transaction, NULL);
    ExtractTscTX tscTx;
    BOOST_CHECK(converter.extractionTscTransactions(tscTx));
    std::vector<TscTransaction> result = tscTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(tsctxconverter_tests, TestingSetup)

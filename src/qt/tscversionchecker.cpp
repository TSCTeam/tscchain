...
#define paternVersion "tsc-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

TscVersionChecker::TscVersionChecker(QObject *parent) : QObject(parent)
{
...
bool TscVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> TscVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(TSC_RELEASES)));
    QEventLoop event;
...
Version TscVersionChecker::getMaxReleaseVersion()
{

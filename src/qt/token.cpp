...
bool ToHash160(const std::string& strTscAddress, std::string& strHash160)
{
    CBitcoinAddress tscAddress(strTscAddress);
    if(tscAddress.IsValid()){
        CKeyID keyid;
        tscAddress.GetKeyID(keyid);
...
bool ToTscAddress(const std::string& strHash160, std::string& strTscAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CBitcoinAddress tscAddress;
    tscAddress.Set(keyid);
    if(tscAddress.IsValid()){
        strTscAddress = tscAddress.ToString();
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToTscAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToTscAddress(tokenEvent.receiver, tokenEvent.receiver);
